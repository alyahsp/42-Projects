...
/*   Updated: 2017/03/20 03:29:50 by angavrel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	has_reached_borders(t_filler *f, int b[f->max.y][f->max.x])
{
	...
	if (f->turn > (f->max.y * f->max.x) / (f->max.y + f->max.x))
		f->goal = 0;
	else if ((f->goal & 1) && has_reached_top(f, b))
		f->goal &= ~1;
	else if ((f->goal & 8) && has_reached_bot(f, b))
		f->goal &= ~8;
	if ((f->goal & 2) && has_reached_left(f, b))
		f->goal &= ~2;
	else if ((f->goal & 4) && has_reached_right(f, b))
		f->goal &= ~4;
}
